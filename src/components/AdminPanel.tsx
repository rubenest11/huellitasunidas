...
const ShelterProfileEditor: React.FC<{
  ...
  const handleDeleteShelter = async () => {
    ...
    try {
      ...
      // Eliminar de shelterData (datos principales)
      const updatedShelterData = { ...shelterData };
      delete updatedShelterData[currentShelter.id];
      setShelterData(updatedShelterData);
      
      // Eliminar de shelters registrados
      const existingShelters = JSON.parse(localStorage.getItem('huellitasUnidas_shelters') || '{}');
      delete existingShelters[currentShelter.id];
      localStorage.setItem('huellitasUnidas_shelters', JSON.stringify(existingShelters));
      
      // Eliminar información pública del refugio
      const publicShelters = JSON.parse(localStorage.getItem('huellitasUnidas_publicShelters') || '{}');
      delete publicShelters[currentShelter.id];
      localStorage.setItem('huellitasUnidas_publicShelters', JSON.stringify(publicShelters));
      
      // Mostrar mensaje de éxito
      alert('Refugio eliminado permanentemente del sistema.');
      ...
      alert('Error al eliminar el refugio. Intenta de nuevo.');
    } finally {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-2xl p-8 w-full max-w-md">
        <div className="text-center mb-6">
          ...
          <h3 className="text-2xl font-bold text-red-800 mb-2">⚠️ Confirmar Eliminación</h3>
          <p className="text-gray-600">
            Esta acción eliminará permanentemente el refugio <strong>{currentShelter.name}</strong> y todos sus datos.
          </p>
        </div>

        <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
          <h4 className="font-semibold text-red-700 mb-2">Se eliminarán:</h4>
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            className={`flex-1 px-4 py-3 rounded-lg font-semibold transition-colors flex items-center justify-center gap-2 ${
              deleteConfirmationText === 'ELIMINAR' && !deletingAccount
...
export const AdminPanel: React.FC<AdminPanelProps> = ({ onBack, shelterData, setShelterData }) => {
  ...
  const [activeTab, setActiveTab] = useState<'donations' | 'adoptions' | 'shelter' | 'shelters'>('donations');
  ...
  const handleUpdateShelterProfile = (updatedData: any) => {
    ...
    try {
      // Actualizar en shelterData principal
      const newShelterData = { ...shelterData };
      if (!newShelterData[currentShelter.id]) {
        newShelterData[currentShelter.id] = { donations: [], dogs: [] };
      }
      
      // Guardar en localStorage para refugios registrados
      const existingShelters = JSON.parse(localStorage.getItem('huellitasUnidas_shelters') || '{}');
      existingShelters[currentShelter.id] = updatedData;
      localStorage.setItem('huellitasUnidas_shelters', JSON.stringify(existingShelters));
